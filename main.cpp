/**
 *  BA Example for windows(linux)
 *  forked from github.com/gaoxiang12/g2o_ba_example
 *  
 *  针对高博在http://www.cnblogs.com/gaoxiang12/p/5304272.html中的例子在最新g2o无法编译的问题
 *  对例子文件进行更新，使其在最新版本g2o和windows平台下都能编译运行。
 *  未删除任何原代码，将替换的语句注释掉了，可以对比修改的地方。
 */
...
int main(int argc, char** argv) {
  ...
  cout << "Found" << pts1.size() << "groups matched points." << endl;
  // 构造g2o中的图
  // 先构造求解器
  ...
  // 使用Cholmod中的线性方程求解器 - change to csparse
  ...
  std::unique_ptr<g2o::BlockSolver_6_3::LinearSolverType> linearSolver =
    std::make_unique<g2o::LinearSolverCSparse<g2o::BlockSolver_6_3::PoseMatrixType>>();

  // 6*3 的参数
  /*g2o::BlockSolver_6_3* block_solver = new g2o::BlockSolver_6_3(linearSolver);*/
  // use std::move here to transfer ownership from linearSolver (which is abandoned hereafter) to blockSolver
...
int findCorrespondingPoints(const cv::Mat& img1, const cv::Mat& img2,
                            vector<cv::Point2f>& points1, vector<cv::Point2f>& points2) {
  // OpenCV 3.x changes ORB APIs...
  cv::Ptr<cv::ORB> orb = cv::ORB::create();
  std::vector<cv::KeyPoint> kp1, kp2;
  cv::Mat desp1, desp2;
  // ORB::operator() is deprecated, use Feature2D::detectAndCompute instead
  ...
  cout << "Found " << kp1.size() << " and " << kp2.size() << " key points respectively." << endl;
